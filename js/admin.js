...
// 1. 状態管理 (STATE)
// ※ CONSTANTS, db, colRefs は config.js で定義済みのため再定義しません。
const STATE = {
    logs: [],
    filteredLogs: [],
    players: [],
...
const UI = {
    ...
    showConfirm: function (message, onConfirm) {
        ...
        if (!modal) return;
        // messageにHTMLタグ(brなど)が含まれる前提の関数なので、呼び出し元で安全に構築する
...
async function initApp() {
    ...
    if (localStorage.getItem('theme') === 'dark') {
        ...
        if (tToggle) tToggle.innerHTML = '☀️';
    }
    ...
    try {
        ...
        if (statusEl) {
            ...
            statusEl.className = 'status-badge status-cloud';
        }
...
function setupListeners() {
    ...
    colRefs.settings.doc('general').onSnapshot(doc => {
        if (doc.exists) {
            ...
            STATE.careOptions = CONSTANTS.DEFAULT_CARES;
        }
    });
    ...
    colRefs.edu.onSnapshot(snapshot => {
        STATE.education = [];
        snapshot.forEach(doc => {
            STATE.education.push({ id: doc.id, ...doc.data() });
        });
...
function filterAndRenderTable() {
    ...
    STATE.filteredLogs = STATE.logs.filter(log => {
        ...
        if (searchInput && !matchName && !matchSoreness) return false;
    ...
    if (viewTeamTrend && viewTeamTrend.style.display === 'block') drawTeamTrendChart(STATE.filteredLogs);
    const viewHeatmap = document.getElementById('view-heatmap');
    if (viewHeatmap && viewHeatmap.style.display === 'block') updateHeatmap(STATE.filteredLogs);
    const viewSprint = document.getElementById('view-sprint');
    if (viewSprint && viewSprint.style.display === 'block') updateSprintRanking();
    const viewFv = document.getElementById('view-fv');
    if (viewFv && viewFv.style.display === 'block') updateFvGrouping(STATE.filteredLogs);
}
...
function updateTodayView() {
    ...
    const missingCountEl = document.getElementById('summary-missing-count');
    if (missingCountEl) missingCountEl.textContent = `${missingPlayers.length} 名`;
    const missingListEl = document.getElementById('summary-missing-list');
    // textContent を使用することで安全に表示
    if (missingListEl) missingListEl.textContent = missingPlayers.length > 0 ? missingPlayers.join(', ') : '全員完了！✨';

    const highFatigue = todayLogs.filter(log => parseInt(log.fatigue) >= CONSTANTS.THRESHOLDS.HIGH_FATIGUE);
    const fatCountEl = document.getElementById('summary-fatigue');
    if (fatCountEl) fatCountEl.textContent = `${highFatigue.length} 名`;
    const fatListEl = document.getElementById('summary-fatigue-list');
    if (fatListEl) fatListEl.textContent = highFatigue.map(l => l.playerName).join(', ');
    ...
    const sleepCountEl = document.getElementById('summary-sleep');
    if (sleepCountEl) sleepCountEl.textContent = `${badSleep.length} 名`;
    const sleepListEl = document.getElementById('summary-sleep-list');
    if (sleepListEl) sleepListEl.textContent = badSleep.map(l => l.playerName).join(', ');
    ...
    const monoCountEl = document.getElementById('summary-monotony-count');
    if (monoCountEl) monoCountEl.textContent = `${highMonotonyPlayers.length} 名`;
    const monoListEl = document.getElementById('summary-monotony-list');
    if (monoListEl) monoListEl.textContent = highMonotonyPlayers.length > 0 ? highMonotonyPlayers.join(', ') : '該当なし';

    const acwrCountEl = document.getElementById('summary-acwr-count');
    if (acwrCountEl) acwrCountEl.textContent = `${dangerACWRPlayers.length} 名`;
    const acwrListEl = document.getElementById('summary-acwr-list');
    if (acwrListEl) acwrListEl.textContent = dangerACWRPlayers.length > 0 ? dangerACWRPlayers.join(', ') : '該当なし';
...
function renderTableData(logsToRender, tbodyId, showDate) {
    ...
    displayLogs.forEach(function (log) {
        ...
        // 変数のサニタイズ（無害化）
        const safeFatigue = escapeHTML(log.fatigue);
        const safeStress = escapeHTML(log.stress);
        
        const fatigueText = (parseInt(log.fatigue) >= CONSTANTS.THRESHOLDS.HIGH_FATIGUE) 
            ? `<span style="color:var(--color-danger);font-weight:900; background:var(--color-danger-light); padding:2px 6px; border-radius:6px;">${safeFatigue} ⚠️</span>` 
            : `<span style="font-weight:800;">${safeFatigue || '-'}</span>`;
        const stressText = (parseInt(log.stress) >= CONSTANTS.THRESHOLDS.HIGH_FATIGUE) 
            ? `<span style="color:var(--color-danger);font-weight:900; background:var(--color-danger-light); padding:2px 6px; border-radius:6px;">${safeStress} ⚠️</span>` 
            : `<span style="font-weight:800;">${safeStress || '-'}</span>`;

        let sorenessHtml = log.soreness ? escapeHTML(log.soreness).split(',').map(p => p.trim() ? `<span class="tag-text">${p.trim()}</span><br>` : '').join('') : '-';
        let sorenessPostHtml = log.sorenessPost ? escapeHTML(log.sorenessPost).split(',').map(p => p.trim() ? `<span class="tag-text-post">${p.trim()}</span><br>` : '').join('') : '-';

        let injuryPreHtml = log.injuryPre ? `<div style="color:var(--secondary); font-size:12px; margin-bottom:4px;"><b>[朝]</b> ${escapeHTML(log.injuryPre)}</div>` : '';
        let injuryPostHtml = log.injury ? `<div style="color:var(--color-danger); font-size:12px;"><b>[夜]</b> ${escapeHTML(log.injury)}</div>` : '';
        ...
        const sleepText = log.sleep ? `<b>${escapeHTML(log.sleep)}h</b> <span style="color:var(--color-warning); font-size:12px;">(${sleepStars})</span>` : '-';
        ...
        if (sLogs.length > 0) {
            sLogs.forEach(s => {
                sprintHtml += `<span style="color:var(--accent); font-weight:900; display:block; margin-bottom: 2px;">${escapeHTML(s.distance)}m : ${escapeHTML(s.time)}s</span>`;
            });
        } else {
            sprintHtml = '-';
        }
        ...
        let safeCare = escapeHTML(String(log.care || '').replace(/null/g, '').split('/').map(s => s.trim()).filter(s => s !== '').join(' / '));

        const commentBtnClass = log.coachComment ? 'btn-success' : 'btn-info';
        const commentBtnText = log.coachComment ? '💬 編集' : '💬 コメント';

        const safePlayerName = escapeHTML(log.playerName);
        const safeDate = escapeHTML(log.date);

        let html = showDate ? `<td style="font-size:13px; font-weight:800; color:var(--text-muted);">${safeDate || ''}</td>` : '';
        html += `
            <td style="font-weight:900; font-size:15px; white-space:nowrap;">${catBadge}${safePlayerName || ''}</td>
            <td>${irsPre}</td>
            <td>${irsPost}</td>
            <td>${fatigueText} / ${stressText}</td>
            <td style="font-weight:900; color:var(--primary); font-size:16px;">${escapeHTML(log.trainingLoad) || ''}</td>
            <td>${sprintHtml}</td>
            <td style="font-weight:800;">${escapeHTML(log.rsi) || ''}</td>
            <td style="font-weight:800;">${escapeHTML(log.fvResult) || ''}</td>
            <td>${sorenessHtml}</td>
            <td>${sorenessPostHtml}</td>
            <td style="max-width:200px; white-space:normal; line-height:1.4;">${injuryCombined}</td>
            <td style="font-weight:800;">${log.weight ? escapeHTML(log.weight) + 'kg' : '-'}</td>
            <td style="font-weight:800;">${escapeHTML(log.heartRate) || '-'}</td>
            <td style="font-weight:800;">${escapeHTML(log.steps) || '-'}</td>
            <td>${sleepText}</td>
            ...
            <td style="text-align:center; min-width:90px;">
                <button class="btn btn-info w-full mb-2" style="padding:6px; font-size:12px; background:var(--text-muted); box-shadow:none;" onclick="openDetailModal('${safePlayerName}', '${safeDate}')">📋 詳細</button>
                <button class="btn ${commentBtnClass} w-full mb-2" style="padding:6px; font-size:12px; box-shadow:none;" onclick="openCommentModal('${safePlayerName}', '${safeDate}')">${commentBtnText}</button>
                <button class="btn btn-danger w-full" style="padding:6px; font-size:12px; box-shadow:none;" onclick="deleteLog('${safePlayerName}', '${safeDate}')">削除</button>
            </td>
        `;
        tr.innerHTML = html;
...
function getIrsBadgeHtml(irsStr) {
    ...
    if (isNaN(val)) return escapeHTML(irsStr);
...
function getPlayerCategory(playerName) {
    ...
    return player ? (player.category || 'BLUE') : 'BLUE';
}
...
function sendBroadcast() {
    ...
    if (!title || !message) {
        UI.showToast('タイトルとメッセージを入力してください', 'warning');
        return;
    }

    // targetはセレクトボックスの固定値なので安全
    UI.showConfirm(`この内容で【${target === 'ALL' ? '全員' : target}】へ送信しますか？`, async () => {
        ...
        try {
            if (colRefs.broadcasts) {
                await colRefs.broadcasts.add(data);
            } else {
            ...
            UI.showToast('送信失敗: ' + e.message, 'error');
        }
    });
}
...
function openBroadcastListModal() {
    renderBroadcastList();
    const m = document.getElementById('broadcast-list-modal');
    if (m) m.style.display = 'flex';
}
...
function renderBroadcastList() {
    ...
    STATE.broadcasts.forEach(b => {
        ...
        // 未読ユーザー名のエスケープ
        const unreadPlayers = targetPlayers.filter(name => !(b.readBy || []).includes(name)).map(name => escapeHTML(name));
        ...
        // 入力文字のサニタイズ
        const safeTarget = escapeHTML(b.target);
        ...
        div.innerHTML = `
            <div class="flex-between mb-3" style="align-items:flex-start;">
                <div>
                    <span class="status-badge" style="margin-left:0; margin-right:8px; background:var(--card-bg); color:var(--text-main); border:1px solid var(--border-color); box-shadow:none;">${safeTarget}宛</span>
                    <span style="font-weight:900; color:var(--primary); font-size:16px;">${safeTitle}</span>
                </div>
                <span style="font-size:12px; color:var(--text-muted); font-weight:800;">${new Date(b.createdAt).toLocaleString()}</span>
            </div>
            <div style="font-size:14px; margin-bottom:16px; color:var(--text-main); white-space:pre-wrap; background:var(--card-bg); padding:14px; border-radius:12px; font-weight:600; box-shadow:var(--shadow-sm);">${safeMessage}</div>
...
async function saveCoachComment() {
    ...
    if (!stamp && !text) {
        UI.showToast('スタンプかメッセージのどちらかを入力してください', 'warning');
        return;
    }
    ...
    try {
        ...
        UI.showToast("フィードバックを送信しました！", "success");
        closeCommentModal();
    } catch (e) {
        UI.showToast('保存失敗: ' + e.message, 'error');
    }
}
...
function drawTeamTrendChart(logs) {
    ...
    const ctxEl = document.getElementById('teamTrendChart');
    if (!ctxEl) return;
...
function updateCharts() {
    const playerSelectEl = document.getElementById('chart-player-select');
    if (!playerSelectEl) return;
    const playerName = playerSelectEl.value;
    if (!playerName) return;
    const baseLogs = STATE.filteredLogs;
    const playerLogs = baseLogs.filter(log => log.playerName === playerName);
    const recent7Logs = playerLogs.slice(0, 7);
    const metrics = calculateMonotony([...recent7Logs].reverse());

    const tLoadEl = document.getElementById('stat-total-load');
    if (tLoadEl) tLoadEl.textContent = Math.round(metrics.totalLoad);
    ...
    const strainEl = document.getElementById('stat-strain');
    if (strainEl) strainEl.textContent = Math.round(metrics.strain);
    ...
    if (recentInjuryList) {
        ...
        recent7Logs.forEach(log => {
            if (log.injuryPre || log.injury) {
                hasInjury = true;
                let text = `<li style="margin-bottom:8px;"><span style="font-weight:900; color:var(--text-muted); margin-right:8px;">${escapeHTML(log.date.split('-').slice(1).join('/'))}</span>`;
                if (log.injuryPre) text += `<span style="color:var(--secondary); font-size:12px;"><b>[朝]</b> ${escapeHTML(log.injuryPre)}</span><br>`;
                if (log.injury) text += `<span style="color:var(--color-danger); font-size:12px;"><b>[夜]</b> ${escapeHTML(log.injury)}</span>`;
                text += `</li>`;
    ...
    if (pRankingBody) {
        ...
        if (pSorted.length === 0) {
            pRankingBody.innerHTML = '<li style="list-style:none; color:var(--text-muted); margin-left:-20px; text-align:center;">直近の訴えなし</li>';
        } else {
            pSorted.forEach(item => {
                pRankingBody.innerHTML += `<li style="margin-bottom:4px;"><span style="font-weight:900; color:var(--text-main); margin-right:8px;">${escapeHTML(item[0])}</span> <span style="color:var(--primary);">(${item[1]}回)</span></li>`;
            });
        }
    }
...
function drawRsiChart(logs) {
    const ctxEl = document.getElementById('rsiChart');
    if (!ctxEl) return;
    const ctx = ctxEl.getContext('2d');
    const labels = logs.map(l => l.date.substring(5));
...
function updateHeatmap(logs) {
    ...
    updateHeatSpot('heat-arm-l-f', sorenessCounts['腕']);
    updateHeatSpot('heat-arm-r-f', sorenessCounts['腕']);
    ...
    updateHeatSpot('heat-lower-back', sorenessCounts['腰']);
    updateHeatSpot('heat-arm-l-b', sorenessCounts['腕']);
    updateHeatSpot('heat-arm-r-b', sorenessCounts['腕']);
    ...
    if (sortedSoreness.length === 0) {
        ...
        sortedSoreness.forEach((item, index) => {
            ...
            rankingBody.innerHTML += `<tr><td style="font-weight:900; color:var(--text-muted);">${index + 1}</td><td style="font-weight:900; font-size:13px;">${escapeHTML(item[0])}${badge}</td><td style="font-weight:900; color:var(--primary); font-size:16px;">${item[1]}</td></tr>`;
        });
    }
}
...
function updateSprintRanking() {
    ...
    sortedPlayers.forEach((item) => {
        ...
        rankingBody.innerHTML += `<tr><td style="font-weight:900; color:var(--text-muted);">${displayRank}</td><td style="font-weight:900; font-size:14px; text-align:left; padding-left:24px; display:flex; align-items:center;">${catBadge}${escapeHTML(playerName)}</td><td style="font-weight:900; color:var(--secondary); font-size:16px;">${time.toFixed(2)}</td></tr>`;
        previousTime = time;
        currentRank++;
    });
}
...
function updateFvGrouping(logs) {
    ...
    if (!lists.force || !lists.balanced || !lists.velocity) return;
    ...
    Object.entries(latestFvData).forEach(([playerName, times]) => {
        ...
        const tagHtml = `<div class="fv-player-tag">${escapeHTML(playerName)} <span class="fv-player-ratio">${ratio.toFixed(2)}</span></div>`;
        if (ratio >= CONSTANTS.THRESHOLDS.FV_FORCE_DEFICIT) {
            lists.force.innerHTML += tagHtml;
            counts.f++;
        } else if (ratio <= CONSTANTS.THRESHOLDS.FV_VELOCITY_DEFICIT) {
            lists.velocity.innerHTML += tagHtml;
            counts.v++;
        } else {
            lists.balanced.innerHTML += tagHtml;
            counts.b++;
        }
    });
...
function toggleTheme() {
    ...
    if (themeEl) themeEl.innerHTML = isDark ? '☀️' : '🌙';
...
function openHistoryView(viewId) {
    const hm = document.getElementById('history-menu');
...
function backToHistoryMenu() {
    document.querySelectorAll('.history-view').forEach(el => el.style.display = 'none');
    const hm = document.getElementById('history-menu');
    if (hm) hm.style.display = 'grid';
}

async function addCareOption() {
    const input = document.getElementById('new-care-input');
    if (!input) return;
...
function renderCareOptions() {
    ...
    STATE.careOptions.forEach((opt, idx) => {
        container.innerHTML += `<div class="tag-text" style="display:flex;align-items:center;gap:8px;padding:10px 14px;font-size:14px; background:var(--card-bg);"><span>${escapeHTML(opt)}</span><span style="color:var(--color-danger);cursor:pointer;font-weight:900;font-size:20px;line-height:1;" onclick="deleteCareOption(${idx})">×</span></div>`;
    });
}
...
function renderPlayersList() {
    ...
    STATE.players.forEach(p => {
        ...
        tag.innerHTML = `${escapeHTML(p.name)} <button class="delete-player-btn" onclick="deletePlayer('${escapeHTML(p.id)}')">×</button>`;
...
function addPlayer() {
    ...
    if (STATE.players.some(p => p.name === name)) {
        UI.showToast('登録済みです', 'warning');
        return;
    }
    const data = { name: name, category: catInput.value, createdAt: new Date().toISOString() };
    if (colRefs.players) colRefs.players.add(data);
    input.value = '';
    UI.showToast("選手を追加しました", "success");
}

function deletePlayer(id) {
    UI.showConfirm("削除しますか？", () => {
        if (colRefs.players) colRefs.players.doc(id).delete();
        UI.showToast("削除しました", "success");
    });
}
...
function renderGoalsTable() {
    ...
    STATE.players.forEach(player => {
        ...
        const seasonGoal = goalData.seasonGoal ? escapeHTML(goalData.seasonGoal) : '<span style="color:var(--text-muted);">未設定</span>';
        const monthGoal = goalData.monthGoal ? escapeHTML(goalData.monthGoal) : '<span style="color:var(--text-muted);">未設定</span>';

        tr.innerHTML = `
            <td style="font-weight:900; font-size:15px; display:flex; align-items:center;">${catBadge}${escapeHTML(player.name)}</td>
            <td><div class="flex-between"><div style="white-space:pre-wrap; font-size:14px; font-weight:800; color:var(--secondary);">${seasonGoal}</div><button onclick="editGoalAdmin('${escapeHTML(player.name)}', 'season')" style="background:none; border:none; color:var(--primary); cursor:pointer; font-size:18px; padding:0; transition:transform 0.2s;">✎</button></div></td>
            <td><div class="flex-between"><div style="white-space:pre-wrap; font-size:14px; font-weight:800; color:var(--primary);">${monthGoal}</div><button onclick="editGoalAdmin('${escapeHTML(player.name)}', 'month')" style="background:none; border:none; color:var(--secondary); cursor:pointer; font-size:18px; padding:0; transition:transform 0.2s;">✎</button></div></td>
        `;
...
async function saveTeamSettings() {
    const nameEl = document.getElementById('target-event-name');
    const dateEl = document.getElementById('target-event-date');
...
async function addEducation() {
    ...
    const catEl = document.getElementById('edu-category');
    const urlEl = document.getElementById('edu-url');
    const descEl = document.getElementById('edu-desc');
    if (!titleEl || !catEl || !urlEl || !descEl) return;
    ...
    if (!title) {
        UI.showToast('タイトルを入力してください。', "warning");
        return;
    }
...
function deleteEducation(id) {
    UI.showConfirm("このコンテンツを削除しますか？", async () => {
        try {
            if (colRefs.edu) await colRefs.edu.doc(id).delete();
            UI.showToast('削除しました', 'success');
        } catch (e) {
...
function renderEducationTable() {
    ...
    STATE.education.forEach(item => {
        const tr = document.createElement('tr');
        const ytLink = item.url ? `<a href="${escapeHTML(item.url)}" target="_blank" style="color:var(--primary); font-size:13px; display:inline-block; margin-top:8px; font-weight:900;">🔗 リンクを開く</a>` : '';
        tr.innerHTML = `
            <td><span class="status-badge" style="background:var(--secondary-alpha); color:var(--secondary); margin:0;">${escapeHTML(item.category)}</span></td>
            <td><strong style="color:var(--secondary); font-size:15px;">${escapeHTML(item.title)}</strong><br><span style="font-size:13px; color:var(--text-main); white-space:pre-wrap; font-weight:600; display:block; margin-top:4px;">${escapeHTML(item.description)}</span>${ytLink}</td>
            <td style="text-align:center;"><button class="btn btn-danger" style="padding: 8px 12px; font-size:12px; box-shadow:none;" onclick="deleteEducation('${escapeHTML(item.id)}')">削除</button></td>
        `;
        tbody.appendChild(tr);
    });
}
...
function deleteLog(playerName, date) {
    UI.showConfirm(`本当に削除しますか？`, async () => {
        const docId = `${playerName}_${date}`;
        if (colRefs.logs) {
            await colRefs.logs.doc(docId).delete();
            UI.showToast('削除しました', 'success');
        }
    });
}
...
function clearAllData() {
    UI.showConfirm("全データを削除しますか？<br><span style='font-size:13px; color:var(--color-danger);'>※この操作は取り消せません。</span>", async () => {
        if (colRefs.logs) {
            const snapshot = await colRefs.logs.get();
...
function downloadCSV() {
    ...
    if (logs.length === 0) {
        UI.showToast("ダウンロードするデータがありません。", "warning");
        return;
    }
    let csvContent = '\uFEFF';
