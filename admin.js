...
async function initApp() {
    setDefaultDates();
    if (localStorage.getItem('theme') === 'dark') { document.body.classList.add('dark-mode'); const tToggle = document.getElementById('theme-toggle'); if(tToggle) tToggle.innerHTML = '☀️';}
...
function updateTodayView() {
    ...
    const missingCountEl = document.getElementById('summary-missing-count'); if(missingCountEl) missingCountEl.textContent = `${missingPlayers.length} 名`;
    const missingListEl = document.getElementById('summary-missing-list'); if(missingListEl) missingListEl.textContent = missingPlayers.length > 0 ? missingPlayers.join(', ') : '全員完了！✨';
    
    const highFatigue = todayLogs.filter(log => parseInt(log.fatigue) >= CONSTANTS.THRESHOLDS.HIGH_FATIGUE);
    const fatCountEl = document.getElementById('summary-fatigue'); if(fatCountEl) fatCountEl.textContent = `${highFatigue.length} 名`;
    const fatListEl = document.getElementById('summary-fatigue-list'); if(fatListEl) fatListEl.textContent = highFatigue.map(l => l.playerName).join(', ');
    ...
    const sleepCountEl = document.getElementById('summary-sleep'); if(sleepCountEl) sleepCountEl.textContent = `${badSleep.length} 名`;
    const sleepListEl = document.getElementById('summary-sleep-list'); if(sleepListEl) sleepListEl.textContent = badSleep.map(l => l.playerName).join(', ');
    ...
    const monoCountEl = document.getElementById('summary-monotony-count'); if(monoCountEl) monoCountEl.textContent = `${highMonotonyPlayers.length} 名`;
    const monoListEl = document.getElementById('summary-monotony-list'); if(monoListEl) monoListEl.textContent = highMonotonyPlayers.length > 0 ? highMonotonyPlayers.join(', ') : '該当なし';
    
    const acwrCountEl = document.getElementById('summary-acwr-count'); if(acwrCountEl) acwrCountEl.textContent = `${dangerACWRPlayers.length} 名`;
    const acwrListEl = document.getElementById('summary-acwr-list'); if(acwrListEl) acwrListEl.textContent = dangerACWRPlayers.length > 0 ? dangerACWRPlayers.join(', ') : '該当なし';
...
// 📌 7. ブロードキャスト & 個別コメント
// ==========================================
function openBroadcastModal() { const m = document.getElementById('broadcast-modal'); if(m) m.style.display = 'flex'; }
function closeBroadcastModal() { const m = document.getElementById('broadcast-modal'); if(m) m.style.display = 'none'; }
...
function openBroadcastListModal() { renderBroadcastList(); const m = document.getElementById('broadcast-list-modal'); if(m) m.style.display = 'flex'; }
function closeBroadcastListModal() { const m = document.getElementById('broadcast-list-modal'); if(m) m.style.display = 'none'; }
...
function openCommentModal(playerName, date) {
    ...
    const m = document.getElementById('comment-modal'); if(m) m.style.display = 'flex';
}

function closeCommentModal() {
    const m = document.getElementById('comment-modal'); if(m) m.style.display = 'none'; currentCommentTarget = null;
}
...
function openDetailModal(playerName, date) {
    ...
    const m = document.getElementById('detail-modal'); if(m) m.style.display = 'flex';
}
function closeDetailModal() {
    const m = document.getElementById('detail-modal'); if(m) m.style.display = 'none';
}
...
function updateCharts() {
    ...
    const tLoadEl = document.getElementById('stat-total-load'); if(tLoadEl) tLoadEl.textContent = Math.round(metrics.totalLoad);
    ...
    const strainEl = document.getElementById('stat-strain'); if(strainEl) strainEl.textContent = Math.round(metrics.strain);
...
function openHistoryView(viewId) {
    const hm = document.getElementById('history-menu'); if(hm) hm.style.display = 'none';
...
function backToHistoryMenu() {
    document.querySelectorAll('.history-view').forEach(el => el.style.display = 'none');
    const hm = document.getElementById('history-menu'); if(hm) hm.style.display = 'grid';
}
...
async function addEducation() {
    const titleEl = document.getElementById('edu-title'); const catEl = document.getElementById('edu-category');
    const urlEl = document.getElementById('edu-url'); const descEl = document.getElementById('edu-desc');
    if(!titleEl || !catEl || !urlEl || !descEl) return;
    
    const title = titleEl.value.trim(); const category = catEl.value;
    const url = urlEl.value.trim(); const desc = descEl.value.trim();
    ...
    try {
        ...
        titleEl.value = ''; urlEl.value = ''; descEl.value = '';
