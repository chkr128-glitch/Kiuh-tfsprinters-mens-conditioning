...
const CONSTANTS = {
    ...
    THRESHOLDS: { 
        ...
        // 🔥 IRS計算用の重み付けを追加
        IRS_WEIGHTS: { FATIGUE_SEVERE: { threshold: 8, score: 15 }, FATIGUE_HIGH: { threshold: 6, score: 5 }, STRESS_HIGH: { threshold: 8, score: 5 }, SLEEP_SHORT: { threshold: 6, score: 10 }, SLEEP_MED: { threshold: 7, score: 5 }, QUALITY_LOW: { threshold: 2, score: 5 }, LOAD_HIGH: { threshold: 600, score: 10 }, RPE_HIGH: { threshold: 8, score: 10 }, CRITICAL_PARTS: ['ハムストリングス', 'カーフ', 'アキレス腱', '腸腰筋', '大腿四頭筋', '腰'], SORENESS_HAMSTRING: 20, SORENESS_CALF_ACHILLES: 10, SORENESS_QUAD_HIP_LOWER: 10, SORENESS_OTHER: 3 }
    },
    COLORS: { CHART_LOAD: '#0ea5e9', CHART_FATIGUE: '#8b5cf6', TEXT_MUTED: '#6b7280', GRID_LINE: '#e5e7eb' },
    DEFAULT_CARES: ["🍚 栄養補給", "🧊 アイシング", "🛁 交代浴", "🧘‍♂️ ストレッチ", "💆‍♂️ ローラー", "💤 睡眠"],
    LEVELS: { info: { bgClass: 'b-info', label: 'お知らせ' }, warning: { bgClass: 'b-warning', label: '重要' }, danger: { bgClass: 'b-danger', label: '緊急' } }
};

const STATE = {
    logs: [], filteredLogs: [], players: [], settings: {}, goals: {}, education: [], careOptions: [], broadcasts: [],
    charts: { load: null, rsi: null, teamTrend: null, reportRadar: null, reportLine: null }
};

let db = null; let colRefs = {};
...
function renderTableData(logsToRender, tbodyId, showDate) {
    ...
    displayLogs.forEach(function(log) {
        ...
        const safeMenu = (log.menu || '-').replace(/'/g, "\\'").replace(/\n/g, '\\n');
        const safeGood = (log.good || '-').replace(/'/g, "\\'").replace(/\n/g, '\\n');
        const safeBad = (log.bad || '-').replace(/'/g, "\\'").replace(/\n/g, '\\n');
